...
case class Term[N <: MPNumber](val coeff: N, val powers: NumVec[N]) {
  ...
  def *(that: N)(implicit mp: MathProcessor[N, _]): Term[N] =
    new Term(this.coeff * that, this.powers)

  def *(that: Term[N])(implicit mp: MathProcessor[N, _]): Term[N] =
  ...
  def unary_-(implicit mp: MathProcessor[N, _]): Term[N] = withCoeff(-coeff)

  def **(that: Int)(implicit mp: MathProcessor[N, _]): Term[N] =
    new Term(this.coeff ** mp.fromInt(that), this.powers * mp.fromInt(that))

  def **(that: Rational)(implicit mp: MathProcessor[N, _]): Term[N] =
    new Term(this.coeff ** mp.fromRational(that), this.powers * mp.fromRational(that))

  def **(that: N)(implicit mp: MathProcessor[N, _]): Term[N] =
    new Term(this.coeff ** that, this.powers * that)

  def root(that: Int)(implicit mp: MathProcessor[N, _]): Term[N] =
    this ** Rational(1, that)
}

object Term {
  def apply[N <: MPNumber](pair: (N, NumVec[N])): Term[N] = Term(pair._1, pair._2)
  def zero[N <: MPNumber](dim: Int)(implicit mp: MathProcessor[N, _]): Term[N] =
    Term(mp.zero, IndexedSeq.fill(dim)(mp.zero))
  def one[N <: MPNumber](dim: Int)(implicit mp: MathProcessor[N, _]): Term[N] =
